// const fs = require('fs')
// const json = require('./w1ord.json')

const getkey=function(index){
  const keys = '0123456789zxcvbnmasdfghjklqwertyuiopZXCVBNMASDFGHJKLQWERTYUIOP'
  let len = keys.length
  // let count = Math.pow(len,2)
  let a = Math.floor(index / len)
  let b = index % len
  return keys[a] + keys[b]
  // return parseInt(keys[a] + keys[b])
}
...
const findKeys = (map,keys)=>{
  let reskey = []
  for(var i=0;i<keys.length/2;i++){
    reskey.push(keys.substr(i*2,2))
  }
  keys = reskey
  let res ={
    keys:'',
...
// fs.writeFile('./try4.txt', res, function(err) {
//   if(err){
//     console.log(err)
//   }
//   console.log('scusess')
